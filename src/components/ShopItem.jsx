  ...
  return (
    ...
    >
      {!confirmDelete ? (
        <>
              ...
              <AnimatePresence>
                {item.comprado && (
                  <motion.div initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }} exit={{ scale: 0 }}>
                    <Check size={14} color="#000" strokeWidth={3} aria-hidden="true" />
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              {pessoas && pessoas.length > 1 && (
                <button
                  ...
                  className="p-2.5 cursor-pointer text-muted hover:text-brand transition-colors"
                  aria-label={t('shopping.assign')}
                >
              ...
              {isOwner && (
                <motion.button
                  ...
                  aria-label={`${t('shopping.remove')} ${item.nome}`}
                  className="p-2.5 text-muted hover:text-brand transition-colors"
                >
          ...
          <AnimatePresence>
            {showCatPicker && (
              ...
              >
                <div className="px-2 py-2 grid grid-cols-5 gap-1" role="group" aria-label={t('shopping.changeCategory')}>
                  {CAT_ORDER.map((k) => {
                    const c = CATS[k]
                    return (
                      <button
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2.5 p-3.5" role="group" aria-label="Confirmar remoção">
